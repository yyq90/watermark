    ...
    for r, dirs, files in os.walk(foldername):
        ...
        for file in files:
            ...
            if _img is not None:
                ...
                # _img = _img[int(np.ceil(_s[0])):(int(np.ceil(_s[0]))+_e[0]), int(np.ceil(_s[1])):(int(np.ceil(_s[1]))+_e[1]), :]
...
def solve_images2(J, W_m, alpha, W_init, gamma=1, beta=1, lambda_w=0.005, lambda_i=1, lambda_a=0.01, iters=4):
    '''
    Master solver, follows the algorithm given in the supplementary.
    W_init: Initial value of W
    Step 1: Image Watermark decomposition
    ...
    for _ in range(iters):
        ...
        for i in range(K):
            ...
            phi_data = Func_Phi_deriv(np.square(alpha * Wk[i] + (1 - alpha) * Ik[i] - J[i]))
            phi_W = Func_Phi_deriv(np.square(np.abs(alpha_gx) * Wkx + np.abs(alpha_gy) * Wky))
            phi_I = Func_Phi_deriv(np.square(np.abs(alpha_gx) * Ikx + np.abs(alpha_gy) * Iky))
            phi_f = Func_Phi_deriv(((Wm_gx - alphaWk_gx) ** 2 + (Wm_gy - alphaWk_gy) ** 2))
            phi_aux = Func_Phi_deriv(np.square(Wk[i] - W))
            phi_rI = Func_Phi_deriv(np.abs(alpha_gx) * (Ikx ** 2) + np.abs(alpha_gy) * (Iky ** 2))
            phi_rW = Func_Phi_deriv(np.abs(alpha_gx) * (Wkx ** 2) + np.abs(alpha_gy) * (Wky ** 2))
