...
def estimate_watermark(foldername):
    ...
    # my=np.fromiter(list(gradx), dtype=np.float64)
    # mt=list(gradx)


    # mx = np.reshape(ml,(ml.__len__(),np.shape(ml[0])[0],np.shape(ml[0])[1],np.shape(ml[0])[2]))
    # ml = np.zeros(shape=((t.__len__(),np.shape(t[0])[0],np.shape(t[0])[1],np.shape(t[0])[2])))
    # for index,value in enumerate(t):
    #     ml[index]=value
    # # mf =np.asarray(ml)
    #
    # float_arr = np.vstack(ml[:, :]).astype(np.float64)
    # mo = np.asarray(np.asarray(list(gradx)))

    Wm_x = np.median(np.array(list(gradx)), axis=0)
    Wm_y = np.median(np.array(list(grady)), axis=0)

    return (Wm_x, Wm_y, gradx, grady,len(images))
