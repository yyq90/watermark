    ...
    for root, dirs, files in os.walk(foldername):
        for file in files:
            ...
            if img is not None:
                ...
                final_img = np.pad(img, ((int(np.ceil(m_t)), int(np.ceil(size - m - int(np.ceil(m_t))))),(int(np.ceil(n_t)), int(np.ceil(size - n - int(np.ceil(n_t))))), (0, 0)),mode='constant')
                # final_img = np.pad(img, ((m_t, size-m-m_t), (n_t, size-n-n_t),0), mode='constant')
...
if __name__ == "__main__":
    ...
    preprocess('d:/Git/automatic-watermark-detection/images2')
